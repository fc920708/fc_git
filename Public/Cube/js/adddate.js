...
"cur_bg"        : "#ccc", //当日日期单元格背影色 
"sel_bg"        : "#FFCC00", //已被选择的日期单元格背影色 2006-12-03 寒羽枫添加 
"sun_word"      : "#FF0000", //星期天文字颜色 
"sat_word"      : "#FF0000", //星期六文字颜色 
"td_word_light" : "#333333", //单元格文字颜色 
"td_word_dark" : "#CCCCCC", //单元格文字暗色 
"td_bg_out"     : "#fff", //单元格背影色 
"td_bg_over"    : "#eee", //单元格背影色 
"tr_word"       : "#000", //日历头文字颜色 
"tr_bg"         : "#fff", //日历头背影色 
...
mvAry[mvAry.length] = ' <div name="calendarForm" style="margin: 0px;margin-top:5px;">'; 
mvAry[mvAry.length] = '    <table width="100%" border="0" cellpadding="0" cellspacing="1" style="font-size:12px;margin-bottom:5px;">'; 
mvAry[mvAry.length] = '      <tr>'; 
mvAry[mvAry.length] = '        <th align="left" width="1%"><input style="border: 0;background:#fff;width:16px;height:20px;'; 
...
mvAry[mvAry.length] = '        <th align="center" width="98%"  style="text-align: center;" nowrap="nowrap"><select name="calendarYear" id="calendarYear" style="font-size:12px;margin-right:5px;"></select><select name="calendarMonth" id="calendarMonth" style="font-size:12px;'; 
if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到年时隐藏 “月” 
mvAry[mvAry.length] = '"></select></th>'; 
mvAry[mvAry.length] = '        <th align="right" width="1%"><input style="border:0;background:#fff;width:16px;height:20px;'; 
if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到年时隐藏 “月” 
...
mvAry[mvAry.length] = '      <select name="calendarHour" id="calendarHour" style="font-size:12px;margin-right:5px;"></select>' + "："; 
mvAry[mvAry.length] = '<span style="' 
if(calendar.DateMode >= pickMode["hour"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到小时时隐藏 “分” 
mvAry[mvAry.length] = '"><select name="calendarMinute" id="calendarMinute" style="font-size:12px;margin-right:5px;"></select>' +'</span>'; 
mvAry[mvAry.length] = '<span style="' 
if(calendar.DateMode >= pickMode["minute"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到小时、分时隐藏 “秒” 
mvAry[mvAry.length] = '"><select name="calendarSecond" id="calendarSecond" style="font-size:12px;margin-right:5px;margin-left:5px;"></select>'+ Calendar.language["second"][this.lang]+'</span>'; 
...
mvAry[mvAry.length] = '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color: #33CCFF;width:60px;height:20px;font-size:12px;cursor:pointer"/>'; 
...
{ 
var str = '<div id="calendarPanel" style="position: absolute;display: none;z-index:9999; background-color: #FFFFFF;border: 1px solid #CCCCCC;width:215px;font-size:12px;border-radius:3px;-moz-box-shadow: 2px 2px 10px #909090;-webkit-box-shadow: 2px 2px 10px #909090;box-shadow:2px 2px 10px #909090;"></div>'; 
if(document.all) 
{ 
