...
(function() {
	...
	var init = function() { (function($) {
			...
			$(function() {
				if(istabs){
					console.log("1123")
					istabs = Number(istabs);
					$(".idTabs").idTabs({start:istabs});
					
					$(".idTabs").children('li').eq(istabs).find('a').click();
					$('html, body').animate({scrollTop:0},0);
				}else{
					$(".idTabs").idTabs();
				}
...
var os={
	...
	android: false,
	version: false,
	isMobile: ua.match(/iPad/i) || ua.match(/iPhone/i) || ua.match(/android/i) || ua.match(/Mobile/i)
};
...
//swiper-footernews
var swiperFooternews = new Swiper('.swiper-footernews',{
	mode: 'horizontal',
    pagination: '.pagination-footernews',
    paginationClickable: true,
    slidesPerView: 1,
	loop:'true',
	keyboardControl:'true'
...
function fixPagesHeight() {
	$('.swiper-pages').css({
		height: $('.swiperDjs').height()-navSwiper.height
	})
}
...
function openMobileslink(){
	if(os.isMobile){

	}else{
		var _html = '<div id="dnsbottomlinks_box" style="position:fixed; width:100%; height:100%; top:0; left:0; z-index:50000;">' +
			'<div style="position:relative; color:#FFF; width:100%; height:100%; text-align:center;">' +
			'<div style="position:relative; width:80%; max-width:900px; margin:106px auto;">' +
			'<span style="position:absolute;top:4px;right:5px; display:inline-block; width:5%; height:12%; cursor:pointer; z-index:1;" id="dnsbottomlinksclose"><img src="./jc/images/qrcode_tbig_close.jpg"></span>' +
			'<img src="./jc/images/qrc_dnsbg.jpg">' +
			'<div style="position:absolute;top:0;left:0; width:100%; height:100%; padding:14% 12%;">' +
			'<div style="display:inline-block; width:33.333%; padding:0 3%; margin: 0 5%;" class="items"><a href="http://112.54.84.4:8082/FLoginLevdeo.aspx" target="_blank"  title="移动用户"><img src="./jc/images/chinaMobile_icon.png"><p style="font-size: 14px;color: #333;margin-top: 10px;">移动用户</p></a></div>' +
			'<div style="display:inline-block; width:33.333%; padding:0 3%; margin: 0 5%;" class="items"><a href="http://122.4.220.97:8082/FLoginLevdeo.aspx" target="_blank"  title="电信用户"><img src="./jc/images/chinaTelecom_icon.png"/><p style="font-size: 14px;color: #333;margin-top: 10px;">电信用户</p></a></div>' +
			'</div>' +
			'</div>' +
			'</div>' +
			'</div>';
		$('body').append(_html);
		$("#dnsbottomlinksclose").click(function(){
...
function setvideo_warp(){
	var video_warp = $('#video_warp');
	if(video_warp){
		var b=1920/890;
		video_warp.css({"height":(winW/b)+"px"});
	}
}

function scrollFixed(value){
	var top = $('.header').height();
	if($(this).scrollTop()>(top-1)){
		if(value=='.subright_sm'){
			$(value).css({"position":"fixed","top":"60px","z-index":"1000"});
		}else{
			$(value).css({"position":"fixed","top":"0","left":"0","z-index":"1000"});
		}
	}else{
		if(value=='.subright_sm'){
			$(value).css({"position":"absolute","top":"0"});
		}else{
...
function closeLay(value){
	$(value).removeClass("show");
}
function openLay(value){
	$(value).addClass("show");
	$(value).find('.blwarp').scrollTop(0);
}



function theplayideo(url){
	var html='<div id="fixedvideo" style="position: fixed;width: 100%;height: 100%;background-color: black;top: 0;z-index: 10000;">' +
		'<div style="display:none;width: 100%;height: 100%;" id="videophone">'+
		'</div>'+
		'<div style="display:none;width: 100%;height: 100%;" id="videopc"></div>'+
		'<div class="close" style="color: #FFF;padding:10px;font-size:15px;position: absolute;right:0px;top: 0;cursor: pointer;">关闭</div>' +
		'</div>';
	$('#fixedvideo').remove();
	$('body').append(html);
	$("#fixedvideo .close").on('click',function(){
		$('#fixedvideo').remove();
	});
	if (!navigator.userAgent.match(/mobile/i)){
		window.getHlsInfo=function(){
			return {"otherpclivestream":url};
		}
		$('#videopc').show();
		$('#videopc').append('<iframe style="width:100%;height:100%;position:absolute;" frameborder="0" src="../jwplayer/play.html?v=1.1"></iframe>');
	}else{
		$("#videophone").show();
		$("#videophone").html('<video src="'+url+'" autoplay style="width:100%;height: 100%;"></video>');
	}
}
...
$(document).ready(function(){
	...
	$(window).scroll(function(){
		...
		var subright_sm=$('.subright_sm');
		if(subright_sm){
			scrollFixed('.subright_sm');
		}
	});
	...
	var weixin=$('.footer .f_contact .share .weixin');
	if(weixin){
		weixin.click(function(event){
			event.preventDefault();
			if($(this).children('.weixinqrcode').is(":visible")){
				$(this).children('.weixinqrcode').fadeOut();
			}else{
				$(this).children('.weixinqrcode').fadeIn();
				setTimeout(function(){
					$(this).children('.weixinqrcode').fadeOut("slow");
				},4500);
			}
		})
	}
	...
	//
	$('.mini_nav').on("click", function(event){
		event.preventDefault();
		if($('.menu').is(":visible")){
			$('.menu').slideUp("slow");
    ...
    /*if (!navigator.userAgent.match(/mobile/i)){
      window.getHlsInfo=function(){
        return {"otherpclivestream":"../videojs/immd.mp4"};
      }
      $('#videopc').show();
      $("#videopcPlay").on('click',function(){
        $(this).hide();
        $('#videopc').append('<iframe style="width:100%;height:100%;position:absolute;" frameborder="0" src="jwplayer/play.html?v=1.1"></iframe>');
      });
    }else{
      ...
      videojs("example_video_1").ready(function(){
        var myPlayer = this;
        myPlayer.src({type:"video/mp4",src:"videojs/immd.mp4"});
        //myPlayer.play();
      });
    }*/


    function movinginwindow(){
      $(".movingIn").each(function(index, element) {
        var _obj=$(this);
        var winH=$(window).height();
        if(_obj.attr('class').indexOf('show')!=-1){
          return true;
        }
        var _thisshow=_obj.offset().top - $(window).scrollTop();
        if(0 < _thisshow && _thisshow < 500){
          $(this).addClass('show');
        }
      });
    }
